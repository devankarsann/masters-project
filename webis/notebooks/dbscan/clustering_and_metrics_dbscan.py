...
class dbscan_clustering_and_metrics:
    ...
    def __init__(self, stemmed, representation, eps, min_samples):
        ...
        print('using matrix_name:', self.matrix_name)
        self.eps = eps
    ...
    def load_data(self):
        ...
        if self.matrix_name in ['tfidf/tfidf_matrix', 'tfidf/stemmed_tfidf_matrix', 'bow/bow_matrix', 'bow/stemmed_bow_matrix']:
            print('adding .npz file extension')
            file_ext = '.npz'
        else:
            print('adding .npy file extension')
            file_ext = '.npy'
        ...
        # loading raw data
        raw_data_file = '../../processed_files/webis/df.pickle'
    ...
    def calculate_intracluster_similarity(self):
        ...
        for i in range(self.num_clusters):
            ...
            self.intra_avg_map[i] = avg
            self.intra_variance_map[i] = variance
    ...
    def calculate_total_hits(self):
        self.total_checks = 0
        self.total_hits = 0
        self.total_hit_percent = 0
        for cluster in range(len(self.cluster_list)):
            cluster_docs = [i[1] for i in self.cluster_list[cluster]]
            ...
            self.total_checks_map[cluster] = checks
            
            hits = len(set(original_docs).intersection(set(paraphrase_docs)))
            self.total_hits += hits
            self.total_hits_map[cluster] = hits
            
            self.total_hit_percent_map[cluster] = hits / (checks if checks > 0 else 999999)
        self.total_hit_percent = self.total_hits / (self.total_checks if self.total_checks > 0 else 999999)
        
    def calculate_paraphrase_hits(self):
        self.paraphrase_checks = 0
        self.paraphrase_hits = 0
        self.paraphrase_hit_percent = 0
        paraphrase_indices = list(self.df_merged['paraphrase'][self.df_merged['paraphrase'] == True].index)
        for cluster in range(len(self.cluster_list)):
            cluster_docs = [i[1] for i in self.cluster_list[cluster] if i[1] in paraphrase_indices]
            original_docs = [i+1 for i in cluster_docs if i % 2 == 0]
            paraphrase_docs = [i for i in cluster_docs if i % 2 == 1]
            
            checks = len(original_docs)
            self.paraphrase_checks += checks
            self.paraphrase_checks_map[cluster] = checks
            
            hits = len(set(original_docs).intersection(set(paraphrase_docs)))
            self.paraphrase_hits += hits
            self.paraphrase_hits_map[cluster] = hits
            
            self.paraphrase_hit_percent_map[cluster] = hits / (checks if checks > 0 else 999999)
        self.paraphrase_hit_percent = self.paraphrase_hits / (self.paraphrase_checks if self.paraphrase_checks > 0 else 999999)
     
    def calculate_plagiarize_hits(self):
        self.plagiarize_checks = 0
        self.plagiarize_hits = 0
        self.plagiarize_hit_percent = 0
        plagiarize_indices = list(self.df_merged['paraphrase'][self.df_merged['paraphrase'] == False].index)
        for cluster in range(len(self.cluster_list)):
            cluster_docs = [i[1] for i in self.cluster_list[cluster] if i[1] in plagiarize_indices]
            original_docs = [i+1 for i in cluster_docs if i % 2 == 0]
            plagiarize_docs = [i for i in cluster_docs if i % 2 == 1]
        
            checks = len(original_docs)
            self.plagiarize_checks += checks
            self.plagiarize_checks_map[cluster] = checks
            
            hits = len(set(original_docs).intersection(set(plagiarize_docs)))
            self.plagiarize_hits += hits
            self.plagiarize_hits_map[cluster] = hits
        
            self.plagiarize_hit_percent_map[cluster] = hits / (checks if checks > 0 else 999999)
        self.plagiarize_hit_percent = self.plagiarize_hits / (self.plagiarize_checks if self.plagiarize_checks > 0 else 999999)
...
class gv_model(dbscan_clustering_and_metrics):
    def __init__(self, stemmed, eps, min_sample):
        super().__init__(stemmed, 'glove', eps, min_sample)
